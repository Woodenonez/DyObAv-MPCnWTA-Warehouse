...
for kt in range(MAX_SIM_TIME):
    ...
    # hypos_list_all = mmp_interface.get_motion_prediction(past_traj_NN, ref_map, config_dwa.N_hor, sim_config.scale2nn, batch_size=5)
    # for human in human_list[1:]:
    #     past_traj_NN = [ct2real(x.tolist(), False) for x in human.past_traj]
    #     hypos_list = mmp_interface.get_motion_prediction(past_traj_NN, ref_map, config_dwa.N_hor, sim_config.scale2nn, batch_size=5)
    ...
    #         n_obs = len(mu_list)
    # full_dyn_obs_list = [[[0, 0, 0, 0, 0, 1]]*config_dwa.N_hor for _ in range(n_obs)]
