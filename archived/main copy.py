...
### Run and Vis
plotter = Plotter(config_mpc.ts, config_mpc.N_hor)
plotter.prepare_plots(occ_map, map_extent=map_extent)

plotter.plot_netgraph(net_graph)
...
for kt in range(MAX_SIM_TIME):
    ...
    ### Vis
    plotter.update_plot(kt, action, cost, mpc_interface.base_speed, mu_list_list, std_list_list, the_obs_list)
    plotter.plot_agent(the_robot, color='r')
    plotter.plot_agent(human_a, color='b')
    plotter.plot_agent(human_b, color='g')
    plotter.plot_references(mpc_interface.ref_path, mpc_interface.ref_traj, pred_states, current_refs)
